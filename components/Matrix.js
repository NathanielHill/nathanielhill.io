  ...
  animationUpdate() {
    ...
    if (this.counter % this.props.speed === 0 && this.inactive) {
      ...
      for (let i = 0, len = this.MatrixArray.length; i < len; i++) {
        this.MatrixArray[i]++;
        const randomTextIndex = Math.floor(Math.random() * this.props.textContent.length);
        const randomText = this.props.textContent[randomTextIndex];
        const textYPostion = this.MatrixArray[i] * this.props.fontSize;
